export function initGenreIndustrie() {
  ...
  d3.dsv(";", CSV_PATH, d => {
    ...
    // Trouver la valeur max pour l’échelle des colonnes
    let maxValue = 0;
    data.forEach(d => {
      years.forEach(year => {
        if (d[year] > maxValue) maxValue = d[year];
      });
    });
  
    const heightScale = d3.scaleLinear()
      .domain([0, maxValue])
      .range([0, 100]);  // hauteur max des colonnes en pixels
    ...
    years.forEach(year => {
      ...
      [["Femmes", vF], ["Hommes", vH]].forEach(([sex, val]) => {
        ...
        // 🔥 Nouvelle colonne en dessous
        col.append("div")
          .attr("class", "bar-column")
          .style("height", `${heightScale(val)}px`);
      });
